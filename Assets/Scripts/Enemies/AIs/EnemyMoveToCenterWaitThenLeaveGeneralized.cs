...
public class EnemyMoveToCenterWaitThenLeaveGeneralized : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        moveTo.Add(new EnemyMoveToMovementBehavior(new Vector3(transform.localPosition.x - 4.5f * modifiers, transform.localPosition.y - 5, 0) + transform.parent.position, 5f, new Vector3(transform.localPosition.x - 4.5f * modifiers, transform.localPosition.y - 4, 0) + transform.parent.position));
        moveTo.Add(new EnemyMoveToMovementBehavior(new Vector3(transform.localPosition.x - 4.5f * modifiers, 12, 0) + transform.parent.position, 3f, new Vector3(transform.localPosition.x - 4.5f * modifiers, transform.localPosition.y - 5, 0) + transform.parent.position));
    }
